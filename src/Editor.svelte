<script lang="ts">
    ...
    import { HPI } from 'codemirror-lang-hpi'
    import { createEventDispatcher, onMount } from 'svelte'
    import { oneDark } from './oneDark'
    import { Backend } from './hpi'
    ...
    let HPIBackend: Backend = undefined

    const HPILinter = linter(async () => {
        let diagnostics: Diagnostic[] = []

        let HPIDiagnostics = HPIBackend.lint(code)

        for (let d of HPIDiagnostics) {
            diagnostics.push(
                Object.create({
                    from: d.span.start.charIdx,
                    to: d.span.end.charIdx,
                    severity: d.level,
                    source: 'HPI Programmanalyse',
                    message: d.message,
                }),
            )
        }
